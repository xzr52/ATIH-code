...
class MyEulerAncestralDiscreteScheduler(EulerAncestralDiscreteScheduler):
    ...
    def step_and_update_noise(
        ...
        prev_sample = sample + derivative * dt
        def adaptive_weight(kl_divergence, base_weight=100, factor=0.1):
            return base_weight * torch.exp(-factor * kl_divergence)
        def normalize_to_gaussian(tensor):
            tensor_mean = tensor.mean(dim=[1, 2], keepdim=True)
            tensor_std = tensor.std(dim=[1, 2], keepdim=True) + 1e-6  # 加小常数以防止标准差为零
            normalized_tensor = (tensor - tensor_mean) / tensor_std
            return normalized_tensor
        device = model_output.device
        # noise = randn_tensor(model_output.shape, dtype=model_output.dtype, device=device, generator=generator)
        # prev_sample = prev_sample + noise * sigma_up
        mean = torch.zeros_like(prev_sample)
        std = torch.ones_like(prev_sample)
        white_noise_dist = dist.Normal(mean, std)
        if sigma_up > 0:
            ...
            # else:
            #     #self.noise_list2[self.step_index] = req_noise
            #     #self.noise_list[self.step_index] = req_noise
            #     losses = []
            #     stats_losses = []
            #     for i in range(80):
            #         n = torch.autograd.Variable(self.noise_list[self.step_index].detach().clone(), requires_grad=True)
            #         latent_mean = n.mean(dim=[1, 2], keepdim=True)  # Keeping dimension for broadcasting
            #         latent_std = n.std(dim=[1, 2], keepdim=True)
            #         if is_nan(latent_mean[0, 0, 0,1]):
            #             break
            #         latent_dist = dist.Normal(latent_mean, latent_std)
            #         stats_loss = dist.kl_divergence(latent_dist, white_noise_dist).mean()
            #         stats_losses.append(stats_loss.item())

            #         loss = torch.norm(n - req_noise.detach())#+50*stats_loss
            #         loss.backward()
            #         losses.append(loss.item())
            #         self.noise_list[self.step_index] -= n.grad.detach() * 1.8
            #         #self.noise_list[self.step_index] = normalize_to_gaussian(self.noise_list[self.step_index])
